...
const app = express();
const PORT = 3000;
let activeAppsScriptUrl = process.env.GOOGLE_APPS_SCRIPT_URL || "https://script.google.com/macros/s/AKfycbyikTK1-U5gkscBrHMXsNjwkEgeyYxMtq5za-X_Rey6WdZ7B7i93nevx9lK3x7SB5t4bA/exec";
...
app.post("/api/sync-google-sheet", async (req: Request, res: Response) => {
  try {
    ...
    if (!sheetId || sheetId.length < 8) {
      return res.status(400).json({
        status: "error",
        message: "Invalid Google Sheet link. Please copy the full link from your browser address bar."
      });
    }
...
app.post("/api/google-sheet/test-script", async (req: Request, res: Response) => {
  ...
  if (!targetUrl || !targetUrl.includes("script.google.com")) {
    return res.status(400).json({
      status: "error",
      message: "Please enter a valid Google Apps Script Web App URL (starts with https://script.google.com/...)"
    });
  }
  ...
  try {
    ...
    if (pingResp.ok) {
      ...
      return res.status(pingResp.status).json({
        status: "error",
        message: `Google Apps Script returned HTTP ${pingResp.status}. Make sure 'Who has access' was set to 'Anyone' during deployment.`
      });
    }
  } catch (err: any) {
    return res.status(500).json({
      status: "error",
      message: `Failed to connect: ${err.name === 'AbortError' ? 'Connection timed out' : err.message}. Verify that the Web App is deployed with access set to Anyone.`
    });
  }
});
