...
export const GoogleSheetSyncModal: React.FC<GoogleSheetSyncModalProps> = ({
  ...
  const [sheetUrl, setSheetUrl] = useState<string>(() => {
    return localStorage.getItem('bathuary_google_sheet_url') || '';
  });
  const [autoSyncEnabled, setAutoSyncEnabled] = useState<boolean>(() => {
    return localStorage.getItem('bathuary_auto_sync_enabled') !== 'false';
  });
  ...
  useEffect(() => {
    ...
    fetch('/api/google-sheet/script-url')
      .then(res => res.json())
      .then(data => {
        if (data.status === 'success' && data.scriptUrl) {
  ...
  const handleSaveAndTestGas = async () => {
    ...
    if (!trimmed || !trimmed.includes('script.google.com')) {
      setStatusMessage({
        type: 'error',
        text: 'Please enter a valid Google Apps Script Web App URL ending in /exec'
      });
      return;
    }
  ...
  // Handle Google Sheet Server-Side Proxy Sync (bypasses CORS and follows redirects!)
  const handleFetchGoogleSheet = async () => {
    ...
    try {
      ...
      if (res.ok && data.status === 'success' && Array.isArray(data.beneficiaries)) {
        localStorage.setItem('bathuary_google_sheet_url', sheetUrl.trim());
        localStorage.setItem('bathuary_auto_sync_enabled', String(autoSyncEnabled));
      ...
      // If server returned a permission error message
      if (data && data.message) {
        throw new Error(data.message);
      }

      throw new Error('Could not access Google Sheet. Please check "Share" > "Anyone with the link can view".');
    } catch (err: any) {
      setStatusMessage({
        type: 'error',
              ...
              <div className="mt-2.5 flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem('bathuary_auto_sync_enabled', String(e.target.checked));
                  }}
