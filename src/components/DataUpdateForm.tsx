...
export const DataUpdateForm: React.FC<DataUpdateFormProps> = ({
  ...
  const filteredJobCards = useMemo(() => {
    ...
    return beneficiaries
      .filter(b => b.colH.toLowerCase().includes(q) || b.colJ.toLowerCase().includes(q) || (b.colB && b.colB.toLowerCase().includes(q)))
      .slice(0, 25);
  ...
  const availableIfscs = bankMaster
    .filter(b => !formData.colAO || b.bank.toUpperCase() === formData.colAO.toUpperCase())
    .map(b => b.ifsc);
  ...
  const handleJobCardInputChange = (val: string) => {
    ...
    const exact = beneficiaries.find(b => b.colH.toUpperCase() === val.trim().toUpperCase());
    if (exact) {
